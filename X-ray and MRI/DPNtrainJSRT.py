...
if __name__ == '__main__':
	...
	if os.path.exists(os.path.join(save_dir, 'DPLNetJSRT1e3.pth.tar')):
		# load existing model
		model_info = torch.load(os.path.join(save_dir, 'DPLNetJSRT1e3.pth.tar'))
		print('==> loading existing model:', os.path.join(save_dir, 'DPLNetJSRT1e3.pth.tar'))
	...
	train_dataset = Real('./data/JSRT_train/', 197, args.ps)
	val_dataset = Real('./data/JSRT_val/', 50, args.ps)
	...
	for epoch in range(cur_epoch, args.epochs + 1):
		...
		torch.save({
			...
			os.path.join(save_dir, 'DPLNetJSRT1e3.pth.tar'))
