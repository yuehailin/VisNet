...
from model.cbdnet import Network,NetworkBeforeLGN,NetworkBeforeBLIB, fixed_loss
...
def train(train_loader, val_loader, model, criterion, optimizer):
	...
	with torch.no_grad():
		for (noise_img_val, clean_img_val, sigma_img_val, flag_val) in val_loader:
			...
			noise_level_est, output = model(input_var_val)

			loss_val = criterion(output, target_var_val, noise_level_est, sigma_var_val, flag_var_val)
...
if __name__ == '__main__':
	...
	if os.path.exists(os.path.join(save_dir, 'Load.pth.tar')):
		# load existing model
		model_info = torch.load(os.path.join(save_dir, 'Load.pth.tar'))
		print('==> loading existing model:', os.path.join(save_dir, 'Load.pth.tar'))
	...
	train_dataset = Real('./data/160train/', 160, args.ps)
	val_dataset = Real('./data/160val/', 40, args.ps)
	...
	for epoch in range(cur_epoch, args.epochs + 1):
		...
		torch.save({
			...
			os.path.join(save_dir, 'Load.pth.tar'))
