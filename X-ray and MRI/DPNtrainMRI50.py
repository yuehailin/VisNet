...
if __name__ == '__main__':
	...
	if os.path.exists(os.path.join(save_dir, 'DPLNet_MRI50GP.pth.tar')):
		# load existing model
		model_info = torch.load(os.path.join(save_dir, 'DPLNet_MRI50GP.pth.tar'))
		print('==> loading existing model:', os.path.join(save_dir, 'DPLNet_MRI50GP.pth.tar'))
	...
	train_dataset = Real('./data/MRIGP50_train/', 4712, args.ps)
	val_dataset = Real('./data/MRIGP50_val/', 1000, args.ps)
	...
	for epoch in range(cur_epoch, args.epochs + 1):
		...
		torch.save({
			...
			os.path.join(save_dir, 'DPLNet_MRI50GP.pth.tar'))
